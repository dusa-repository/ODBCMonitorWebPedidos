...
Module Principal
    ...
    Private Sub evaluarPedidos()
        ...
        Try
            ...
            cmdEx.CommandText = "SELECT order_id, salesman_id,  (SELECT top 1 name FROM  salesmen  WHERE  (salesman_id = orders.salesman_id)) AS name, customer_id ,isnull((SELECT  top 1   name     FROM          customers     WHERE      (customer_id = orders.customer_id)),'') as nameC FROM ORDERS WHERE STATUS='PRO' AND DELIVERY_DATE<>'N' and order_date >= " & Year(DateAdd(DateInterval.Day, -5, Now)) & Format(Month(DateAdd(DateInterval.Day, -5, Now)), "00") & Format(Day(DateAdd(DateInterval.Day, -5, Now)), "00") & " and order_date <= " & Year(Now) & Format(Month(Now), "00") & Format(Day(Now), "00") & " ORDER BY order_date DESC"
            ...
            While lrdEx.Read()
                ...
                If buscarPedido(lrdEx.GetInt32(0)) Then
                Else
                    actualizarPedido(lrdEx.GetInt32(0))
                    pedidos = pedidos & "# Pedido:" & lrdEx.GetInt32(0) & "  Vendedor:" & lrdEx.GetString(1) & "," & lrdEx.GetString(2) & " Cliente:" & lrdEx.GetString(3) & "," & lrdEx.GetString(4) & vbCrLf & Environment.NewLine()
                End If
            End While
            conEx.Close()

            If pedidos <> "" Then
                EnvioCorreo("monitorpedidos@dusa.com.ve", "", obtenerDestinatarios(), pedidos)
                'EnvioCorreo("monitorpedidos@dusa.com.ve", "", "lrosales@dusa.com.ve,cdusa@dusa.com.ve", pedidos)
            End If
